...
def build_nasnet_mobile(images, num_classes,
  ...
  # chg add top global scope 
  with tf.variable_scope(None, 'nasnet_mobile', [images]):
...
def build_nasnet_large(images, num_classes,
  ...
  # chg add top global scope 
  with tf.variable_scope(None, 'nasnet_large', [images]):
...
def _build_nasnet_base(images,
  ...
  # print('-----net before final layer', net)
  # Final softmax layer
  with tf.variable_scope('final_layer'):
    net = tf.nn.relu(net)
    # print('-----net after relu', net)
    # chg add 
    add_and_check_endpoint('attention', net)
    net = nasnet_utils.global_avg_pool(net)
    if add_and_check_endpoint('global_pool', net) or num_classes is None:
      return net, end_points
    # print('--net global pool', net)
    net = slim.dropout(net, hparams.dense_dropout_keep_prob, scope='dropout')
    # print('--------net before logits', net)
    # chg add 
    add_and_check_endpoint('PreLogits', net)
