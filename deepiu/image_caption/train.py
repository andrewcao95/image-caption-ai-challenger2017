...
def tower_loss(trainer, input_app=None, input_results=None):
  ...
  if input_results[input_app.input_train_neg_name]:
    if not FLAGS.use_weights:
      ...
      neg_image_name, neg_image_feature, neg_text, neg_text_str, neg_weights = input_results[input_app.input_train_neg_name]
  ...
  if 'neg_weights' in inspect.getargspec(trainer.build_train_graph).args:
    loss = trainer.build_train_graph(image_feature, text, neg_image_feature, neg_text, weights, neg_weights)
  else:
    loss = trainer.build_train_graph(image_feature, text, neg_image_feature, neg_text, weights)
...
def gen_validate(input_app, input_results, validator, predictor):
  ...
  if train_with_validation and not FLAGS.train_only:
    ...
    eval_loss = validator.build_train_graph(eval_image_feature, eval_text, eval_neg_image_feature, eval_neg_text_)
    eval_scores = tf.get_collection('eval_scores')[-1]
    eval_ops = [eval_loss]

    if algos_factory.is_discriminant(FLAGS.algo):
      eval_ops += [validator.correct_ratio]
