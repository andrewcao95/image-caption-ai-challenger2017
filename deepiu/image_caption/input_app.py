...
class InputApp(object):
  def __init__(self):
    ...
    # for scene
    self.scene_input_train_name = 'scene_input_train'
    self.scene_input_valid_name = 'scene_input_valid'
  ...
  def gen_train_input(self, inputs, decode_fn):
    ...
    return results, trainset

  def gen_scene_train_input(self, inputs, decode_fn):
    logging.info('scene_train_input: %s' % FLAGS.scene_train_input)
    trainset = list_files(FLAGS.scene_train_input)
    logging.info('scene_trainset:{} {}'.format(len(trainset), trainset[:2]))
    ...
    results = inputs(
      ...
      num_prefetch_batches=FLAGS.num_prefetch_batches,
      min_after_dequeue=FLAGS.min_after_dequeue,
      name=self.scene_input_train_name)
  ...
  def gen_train_neg_input(self, inputs, decode_neg_fn, trainset):
    ...
    if FLAGS.use_weights:
      neg_weights = results[-1]
      results = input.reshape_neg_tensors(results[:-1], FLAGS.batch_size, FLAGS.num_negs) 
      results.append(neg_weights)
    else:
  ...
  def gen_valid_input(self, inputs, decode_fn):
    ...
    if not FLAGS.use_weights:
      eval_image_name, eval_image_feature, eval_text, eval_text_str = results 
    else:
      eval_image_name, eval_image_feature, eval_text, eval_text_str, _ = results 
    ...
    if FLAGS.show_eval:
      ...
      if eval_fixed_images:
        ...
        if not FLAGS.use_weights:
          fixed_image_name, fixed_image_feature, fixed_text, fixed_text_str = results
        else:
          fixed_image_name, fixed_image_feature, fixed_text, fixed_text_str, _ = results
  ...
  def gen_scene_valid_input(self, inputs, decode_fn):
    #---------------------- valid  
    validset = list_files(FLAGS.scene_valid_input)
    logging.info('scene_validset:{} {}'.format(len(validset), validset[:2]))
    results = inputs(
      validset, 
      decode_fn=decode_fn,
      batch_size=FLAGS.eval_batch_size,
      ...
      min_after_dequeue=FLAGS.min_after_dequeue,
      fix_sequence=FLAGS.fix_sequence,
      name=self.scene_input_valid_name)
    return results

  def gen_valid_neg_input(self, inputs, decode_neg_fn, trainset, eval_batch_size):
    # for eval always use 1 neg sample
    #num_negs = 1 
    num_negs = FLAGS.num_negs
    results = inputs(
      trainset, 
      decode_fn=decode_neg_fn,
      batch_size=eval_batch_size * num_negs,
      num_threads=FLAGS.num_threads,
      batch_join=FLAGS.batch_join,
      shuffle_files=FLAGS.eval_shuffle_files,
      fix_random=FLAGS.eval_fix_random,
    ...
    if FLAGS.use_weights:
      # ignore weights, weights only used for train
      results = results[:-1]
  ...
  def gen_input(self, train_only=False, valid_only=False):
    ...
    inputs, decode_fn, decode_neg_fn = input.get_decodes(use_neg=(FLAGS.num_negs > 0))

    if FLAGS.scene_train_input:
      from deepiu.image_classification import input as scene_input
      scene_inputs, scene_decode_fn, _ = scene_input.get_decodes(use_neg=False) 
    ...
    if not valid_only:
      ...
      # for scene train
      if FLAGS.scene_train_input:
        input_results[self.scene_input_train_name] = self.gen_scene_train_input(scene_inputs, scene_decode_fn)
    ...
    if not train_only:
      ...
      #--for scene valid
      scene_train_with_validation = bool(FLAGS.scene_valid_input)
      if scene_train_with_validation:
        input_results[self.scene_input_valid_name] = self.gen_scene_valid_input(scene_inputs, scene_decode_fn)
