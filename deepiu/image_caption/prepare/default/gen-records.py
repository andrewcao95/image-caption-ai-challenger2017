...
#--deepiu/input_flags.py
#flags.DEFINE_boolean('use_weights', False, 'if True weights_index is text_index + 1')
...
flags.DEFINE_string('image_feature_dir', None, '')
...
def has_weights(l):
  if FLAGS.text_index + 1 >= len(l):
    return False
  if len(l[FLAGS.text_index + 1].split(',')[0].split(':')) == 2:
    return True 
  return False

def get_weights_map(l):
  weights_map = {}
  use_weights = FLAGS.use_weights or has_weights(l)
  if use_weights:
    weights_index = FLAGS.text_index + 1
    weights_list = [item.split(':') for item in l[weights_index].split(',')]
    weights_map = dict([(x, float(y)) for x, y in weights_list])
  return weights_map
...
def deal_imgtextfile(file):
  ...
  convert and store image binaries from im2text(preprocess)
  """
  ...
  with melt.tfrecords.Writer(out_file, FLAGS.buffer_size) as writer:
    num = 0
    for line in open(file):
      if num % 1000 == 0:
      ...
      for text in texts:
        ...
        if weights_map:
          words = [vocab.key(wid) for wid in word_ids]
          weights = map(lambda w: weights_map[w] if w in weights_map else 0, words)
        ...
        if not FLAGS.write_sequence_example:
          ...
          if weights_map:
            feature_dict['weights'] = melt.float_feature(weights)
          ...
          if weights_map:
            feature_dict['weights'] = melt.float_feature_list(weights)
          example = tf.train.SequenceExample(
            ...
            feature_lists=melt.feature_lists(feature_dict))
    ...
    #TODO check why exist with not work?
    writer.close()
...
def deal_file_with_imgdir(file):
  ...
  with melt.tfrecords.Writer(out_file, FLAGS.buffer_size) as writer:
    ...
    for line in open(file):
      ...
      if decoder.decode(encoded_image, image_format=None) is None:
        print('bad image:', image_path, file=sys.stderr)
        continue
...
def deal_file_with_imgfeaturedir(file):
  out_file = '{}/{}'.format(FLAGS.output_directory, '-'.join([FLAGS.name, file.split('/')[-1].split('-')[-1]]))
  print('file:', file, 'out_file:', out_file, file=sys.stderr)
  with melt.tfrecords.Writer(out_file, FLAGS.buffer_size) as writer:
    num = 0
    for line in open(file):
      ...
      img_ = img.replace('/', '_')
      img_ = img_[: img_.rindex('.')]
      image_feature_path =  os.path.join(FLAGS.image_feature_dir, img_ + '.bin')
      image_feature = pickle.load(open(image_feature_path, 'wb'))
...
def run():
  ...
  if FLAGS.info_dir and os.path.exists(FLAGS.info_dir):
    ...
    if not FLAGS.buffer_size:
      #3w 56G
      FLAGS.buffer_size = int(30000 * 5 / 6 / FLAGS.threads)
    read_pic_info()
    ...
    if not FLAGS.buffer_size:
      FLAGS.buffer_size = int(30000 * 5 / 6 / FLAGS.threads)
  elif FLAGS.image_feature_dir and os.path.exists(FLAGS.image_feature_dir):
    print('deal file with imgfeaturedir', file=sys.stderr)
    deal_file_fn = deal_file_with_imgfeaturedir 
    if not FLAGS.buffer_size:
      FLAGS.buffer_size = int(30000 * 5 * 2 * 1536 / IMAGE_FEATURE_LEN / FLAGS.threads)
  else:
    print('normal deal file', file=sys.stderr)
    deal_file_fn = deal_file
    if not FLAGS.buffer_size:
      #3w 4.8G 
      FLAGS.buffer_size = int(30000 * 5 * 2 * 1536 / IMAGE_FEATURE_LEN / FLAGS.threads)

  print('buffer_size:', FLAGS.buffer_size, file=sys.stderr)
