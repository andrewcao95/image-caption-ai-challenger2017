...
if not gezi.non_empty(result_file) or len(open(result_file).readlines()) != 30000:
  if feature_name:
    command = 'python /home/gezi/mine/hasky/deepiu/tools/evaluate-full-inference.py %s %s %d %s' % (model_path, input_file, batch_size, feature_name)
  else:
    command = 'python /home/gezi/mine/hasky/deepiu/tools/evaluate-full-inference.py %s %s %d' % (model_path, input_file, batch_size)
  print(command, file=sys.stderr)
  os.system(command)
else:
  print('%s exists' % result_file)

metrics_file = result_file.replace('.full-evaluate-inference.txt', '.full-caption-metrics.txt')
