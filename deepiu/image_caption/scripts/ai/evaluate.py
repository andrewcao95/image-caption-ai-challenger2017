...
flags.DEFINE_string('valid_resource_dir', '../../../mount/temp/image-caption/ai-challenger/tfrecord/seq-basic/valid/', '')
...
def main(_):
  ...
  plog_dir = '../../../monitor_log'

  sess = tf.Session()
  summary_writer = tf.summary.FileWriter(plog_dir, sess.graph)
  ...
  if not os.path.exists(prediction_file):
    print('not find %s' % prediction_file, file=sys.stderr)
    return
  ...
  print(metric_score_str, os.path.basename(prediction_file), sep='\t', file=open('%s/log.html' % log_dir , 'a'))
  print(metric_score_str, os.path.basename(prediction_file), sep='\t', file=sys.stderr)
