...
from deepiu.util.rank_loss import dot, compute_sim, pairwise_loss, normalize, pairwise_correct_ratio
...
class DiscriminantTrainer(object):
  ...
  def process(self, image_feature):
    ...
    #--if is attention input and just want to use final feature like from 9304 -> 1536 feature dim
    if FLAGS.image_features2feature:
      ...
      image_feature = deepiu.seq2seq.image_encoder.features2feature(image_feature, self.is_training)
    return image_feature

  def forward_image_layers(self, image_feature):
    image_feature = self.process(image_feature)
    dims = self.image_mlp_dims
    if not dims:
      return image_feature

    return melt.slim2.mlp(image_feature, 
                         dims, 
                         self.activation, 
