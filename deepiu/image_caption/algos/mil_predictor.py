...
"""
TODO
duplicate code with dicriminant predictor
"""
...
from deepiu.util.rank_loss import normalize
...
class MilPredictor(MilTrainer, melt.PredictorBase):
  def __init__(self, sess=None):
    #super(DiscriminantPredictor, self).__init__()
    melt.PredictorBase.__init__(self, sess=sess)
    MilTrainer.__init__(self,  is_training=False, is_predict=True)
  ...
  def build_graph(self, image_feature, text):
    with tf.variable_scope(self.scope):
      ...
      #score = self.compute_image_text_sim(image_feature, text_feature, text, combiner='max')
      score = self.compute_image_text_sim(image_feature, text_feature, text)
      return score
  ...
  def build_textsim_graph(self, text,  text2):
    with tf.variable_scope(self.scope):
      ...
      score = self.compute_image_text_sim(text_feature, text_feature2, text2)
      return score
  ...
  def build_fixed_text_feature_graph(self, text_feature_npy): 
    ...
    with tf.variable_scope(self.scope):
      ...
      score = self.compute_image_text_sim(image_feature, text_feature)
      return score
  ...
  def build_train_graph(self, image_feature, text, neg_text, lookup_negs_once=False):
    ...
    """
    return super(MilPredictor, self).build_graph(image_feature, text, neg_text, lookup_negs_once)
  ...
  def forward_word_feature(self):
    ...
    return word_feature, word_index
  ...
  def build_words_importance_graph(self, text):
    with tf.variable_scope(self.scope):
      ...
      #[1, seq_len]
      score = self.compute_image_text_sim(text_feature, word_feature, words)
      return score
  ...
  #TODO only single image is allowed maybe need tf.map_fn to deal with batch images
  def build_image_words_sim_graph(self):
    with tf.variable_scope(self.scope):
      ...
      image_feature = tf.contrib.seq2seq.tile_batch(image_feature, word_feature_batch_size)
