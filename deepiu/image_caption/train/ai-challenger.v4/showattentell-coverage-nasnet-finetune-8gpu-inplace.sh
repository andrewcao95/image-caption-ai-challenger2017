...
model_dir=$base/temp/image-caption/ai-challenger/model.v4/showattentell.coverage.nasnet.finetune.8gpu.inplace
...
#here set to 0.1 if it can not convergent might change to use 0.05
 ...
 # --learning_rate_epoch_boundaries=5,7,9,11 \
python ./train.py \
  ...
  --image_checkpoint_file=$base/'data/image_model_check_point/nasnet_large/model.ckpt' \
  ...
  --use_finetune_step=1 \
  --learning_rate_values=0.1,0.005 \
  --learning_rate_epoch_boundaries=7 \
  --num_epochs=100 \
  --num_sampled 0 \
