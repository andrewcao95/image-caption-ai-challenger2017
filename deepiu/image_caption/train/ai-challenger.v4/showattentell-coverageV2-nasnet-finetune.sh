...
model_dir=$base/temp/image-caption/ai-challenger/model.v4/showattentell.coverageV2.nasnet.finetune
...
python ./train.py \
  ...
  --coverage_v2_attention_wrapper 1 \
  --eval_rank 0 \
  --eval_translation 1 \
  ...
  --use_finetune_step=1 \
  --learning_rate_values=0.1,0.005,0.001 \
  --learning_rate_epoch_boundaries=2,20 \
  --num_epochs=100 \
  --num_sampled 0 \
