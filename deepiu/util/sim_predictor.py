...
class SimPredictor(object):
  ...
  def onebyone_predict(self, ltexts, rtexts):
    if self.image_model is not None:
      ltexts = self.image_model.gen_feature(ltexts)
    return self.predictor.onebyone_predict(ltexts, rtexts)

  def bulk_predict(self, ltexts, rtexts, buffer_size=512):
    if self.image_model is not None:
      ltexts = self.image_model.gen_feature(ltexts)
    return self.predictor.bulk_predict(ltexts, rtexts, buffer_size)
  ...
  def top_words(self, image, k=1): 
    ...
    #val = self.sess.run(scores, feed_dict={self.predictor.lfeed: image}) 
