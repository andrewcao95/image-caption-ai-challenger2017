...
flags.DEFINE_float('margin', 0.1, 'margin for rankloss when rank_loss is hinge loss, margin 0.1 will be better then 0.5 for most applications')
...
flags.DEFINE_string('rank_loss_combiner', 'mean', '')
...
def pairwise_correct_ratio(pos_score, neg_score):
  return tf.reduce_mean(tf.to_float((pos_score - neg_score) > 0))

def pairwise_loss(pos_score, neg_scores):
  if FLAGS.loss == LossType.hinge:
    loss = melt.losses.hinge(pos_score, neg_scores, FLAGS.margin, combiner=FLAGS.rank_loss_combiner)
