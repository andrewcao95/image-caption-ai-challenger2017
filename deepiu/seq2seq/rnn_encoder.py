...
class RnnEncoder(Encoder):
  ...
  def __init__(self, is_training=True, is_predict=False):
    ...
    create_rnn_cell = functools.partial(
        ...
        keep_prob=FLAGS.encoder_keep_prob or FLAGS.keep_prob, 
        num_layers=FLAGS.encoder_num_layers or FLAGS.num_layers, 
  ...
  def encode(self, sequence, emb=None, input=None, output_method=None, 
             method=None, embedding_lookup=True):
    if emb is None:
      emb = self.emb 

    #--for debug
    if embedding_lookup:
      sequence, sequence_length, _ = self.pad(sequence)
    else:
