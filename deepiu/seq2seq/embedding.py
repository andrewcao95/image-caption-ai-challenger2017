...
try:
  ...
  from deepiu.util.conf import TEXT_MAX_WORDS
...
def get_or_restore_embedding(name='emb'):
  ...
  if not FLAGS.word_embedding_file:
    logging.info('Word embedding random init or from model_dir and trainable=:{}'.format(FLAGS.finetune_word_embedding))
    emb = get_embedding(
...
def get_position_embedding(name='pos_emb', height=None):
  if FLAGS.position_embedding:
    ...
    pos_emb = embedding.get_embedding(name, height=height or TEXT_MAX_WORDS)
  else:
    pos_emb = None
  return pos_emb
