...
flags.DEFINE_float('start_epoch', 0, '')
...
def main(_):
  ...
  while True:
    ...
    steps = [int(file.split('/')[-1].split('-')[-1]) for file in files]
    #m = dict([(x, y) for (x, y) in zip(epochs, files)])
    files = [x for _, x in sorted(zip(steps, files))]
    for i, file in enumerate(files):
      file_ = file.split('/')[-1]
      if file_ not in visited_checkpoints:
        epoch = float(file_.split('-')[-2])
        step = int(float(file_.split('-')[-1]))
        if FLAGS.start_epoch and epoch < FLAGS.start_epoch:
          continue
        if FLAGS.epoch_interval and int(epoch) % FLAGS.epoch_interval != 0:
          continue
        logging.info('mointor_epoch:%f model:%s, %d model files has been done'%(epoch, file_, len(visited_checkpoints)))
        ...
        if scores:
          melt.add_summarys(summary, scores, metrics, prefix='step')
