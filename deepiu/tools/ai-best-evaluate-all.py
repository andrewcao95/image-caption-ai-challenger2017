...
def main(_):
  ...
  caption_metrics_file = FLAGS.caption_metrics_file or prediction_file.replace('evaluate-inference', 'best_metrics_all')
  ...
  for i, line in enumerate(open(prediction_file)):
    ...
    for caption in captions:
      ...
      meteor_info[img][caption_str] = -1. 
      rouge_info[img][caption_str] = -1. 
  ...
  while True:
    ...
    for scorer, method in scorers:
      ...
      if type(method) == list:
        ...
        logging.info('{}:{}'.format(scorer.method(), score))
    ...
    if out:
      for i in range(len(selected_results)):
        ...
        meteor_info[img][caption] = meteor 
        rouge = scores_list[3][i]
        rouge_info[img][caption] = rouge

        print(img.split('.')[0], caption.replace(' ', ''), bleu_4, cider, meteor, rouge, selected_ref.replace(' ', ''), sep='\t', file=out)
      out.flush()
    ...
    logging.info('round:{} num_imgs:{} bleu4:{} cider:{} meteor:{} rouge:{}'.format(round, len(imgs), calc_best_metric(bleu4_info), calc_best_metric(cider_info), calc_best_metric(meteor_info), calc_best_metric(rouge_info)))
    round += 1
