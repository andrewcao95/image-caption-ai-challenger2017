...
from gflags import *
DEFINE_boolean('norm', True, "")
DEFINE_integer('bins', 10, '')
DEFINE_string('col', '', '')
DEFINE_string('header', False, "")
DEFINE_string('command', '', 'command for melt')
...
def main(argv):
  ...
  if not first_line.startswith('#'):
    ...
    cmd = 'mlt %s %s -c addheader --rank=1'%(input, FLAGS.command) 
    print cmd
    os.system(cmd)
    input = add_filename_suffix(input, 'addheader')

  cmd = 'mlt %s %s -c sd --rank=1'%(input, FLAGS.command)
  print cmd
  os.system(cmd)
