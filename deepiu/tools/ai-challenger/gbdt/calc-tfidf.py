...
if FLAGS.type == 'test':
  ...
  ofile = './ensemble.test.tfidf.txt'
  opkl = './ensemble.test.tfidf.pkl'
else:
  ofile = './ensemble.valid.tfidf.txt'
  opkl = './ensemble.valid.tfidf.pkl'
...
with open(ofile, 'w') as out:
  for i, line in enumerate(open(FLAGS.input_file)):
    ...
    for caption in captions:
      ...
      for word in segged_caption:
        wid = vocab.id(word)
        tf_map.setdefault(wid, 0)
        tf_map[wid] += 1
    for caption in captions:
      segged_caption = segged_captions_map[caption]
      word_ids = [vocab.id(x) for x in segged_caption]
      weights = [tf_map[x] * idf_weights[x] for x in word_ids]
      ...
      words = ' '.join(results[0])
      scores = ' '.join(map(str, results[1]))
      print(img, caption, words, scores, sep='\t', file=out)
