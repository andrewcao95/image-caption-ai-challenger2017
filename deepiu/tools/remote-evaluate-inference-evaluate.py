...
local_model_path = model_path.replace('hdfsmount', 'mount')
local_dir = gezi.dirname(local_model_path)
command = 'mkdir -p %s' % local_dir
print(command, file=sys.stderr)
os.system(command)

result_file = local_model_path + '.evaluate-inference.txt'
...
if not gezi.non_empty(result_file) or len(open(result_file).readlines()) != 30000:
  command = 'python /home/gezi/mine/hasky/deepiu/image_caption/inference/ai-challenger/evaluate-inference.py %s %s' % (model_path, result_file)
  print(command, file=sys.stderr)
  os.system(command)
else:
...
command = 'CUDA_VISIBLE_DEVICES=-1 /home/gezi/mine/hasky/deepiu/tools/fix-evaluate-summary.py %s' % local_dir
print(command)
os.system(command)
