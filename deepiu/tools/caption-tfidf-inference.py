...
flags.DEFINE_integer('bulk_buffer_size', 512, '')
...
flags.DEFINE_string('feature_name', 'attention', '')
flags.DEFINE_string('weight_file', './ensemble.valid.tfidf.pkl', '')
flags.DEFINE_integer('top_n', 3, '')
...
def get_top_ids(ids, tf_map):
  l = [(id, tf_map[id] * idf_weights[id]) for id in ids]
  l.sort(key=operator.itemgetter(1), reverse=True)
  top_ids = [id for (id, weight) in l]
  top_ids = list(OrderedDict.fromkeys(top_ids))[:TOP_N]
  top_ids = gezi.pad(top_ids, MAX_WORDS)
  return top_ids
...
def _predict(predictor, imgs, texts_list, m):
  ...
  sorted_texts_list = []
  for img, texts in zip(imgs, texts_list):
    l = [' '.join(weight_map[img][text].words[:FLAGS.top_n]) for text in texts]
    sorted_texts_list.append(l)
      
  #text_ids_list = [texts2ids(texts) for texts in texts_list]

  text_ids_list = [texts2ids(texts) for texts in sorted_texts_list]

  raw_imgs = np.array(raw_imgs)
  text_ids_list = np.array(text_ids_list)
  print(sum([len(x) for x in text_ids_list]), 'num_imgs_done', num_imgs_done, file=sys.stderr)
  scores_list = predictor.bulk_predict(raw_imgs, text_ids_list, FLAGS.bulk_buffer_size)
...
def main(_):
  ...
  global weight_map
  timer = gezi.Timer('load tfidf', True)
  with open(FLAGS.weight_file) as f:
