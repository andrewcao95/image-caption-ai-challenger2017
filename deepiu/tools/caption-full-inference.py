...
flags.DEFINE_integer('batch_size_', 10, '')
...
def main(_):
  ...
  candidates = {}
  m = {}
  input_file2 = result_file.replace('.full-inference.txt', '.inference.txt').replace('.full-evaluate-inference.txt', '.evaluate-inference.txt')
  for line in open(input_file2):
    l = line.strip().split('\t')
    img, texts, scores = l[0], l[-2], l[-2]  
    if img in done_imgs:
      continue
    texts = texts.split(' ')
    scores = scores.split(' ')
  ...
  for line in open(input_file):
    ...
    if img in done_imgs:
      continue
    texts = texts.split(' ')
    texts = [x in texts if x not in m[img]]
